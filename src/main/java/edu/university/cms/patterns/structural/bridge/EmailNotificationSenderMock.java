...
public class EmailNotificationSenderMock implements NotificationSender {

    private final PatternTraceService traceService;

    public EmailNotificationSenderMock(PatternTraceService traceService) {
        this.traceService = traceService;
    }

    @Override
    public Notification send(User recipient, NotificationMessage message) {
        trace("Sent notification message through mock email channel");
        return new Notification(UUID.randomUUID(), recipient, "[Mock Email] " + message.subject() + " - " + message.body(), Instant.now());
    }
